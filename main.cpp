...
namespace // unnamed, anonymous namespace for internal (local) linkage
{
  ...
  ****************************************************************************/
  ...
  // small function to initialize a database with an empty record, if it doesn't already exist
  template <typename T>
  void initDatabase(T & db) {
	  // try to access the only record in the database
	  // if an exception is thrown, need to insert the first and only record into the database
	  try {
		  db[{}];
	  }
  ...
  void testDbOperations1() {
	  ...
	  // now, get association key databases
	  auto & acDB = Common::AddressCompanyKeyDB::getInstance();
	  auto & ecDB = Common::EmployeeCompanyKeyDB::getInstance();
	  auto & eaDB = Common::EmployeeAddressKeyDB::getInstance();
  ...
  void testDbOperations2() {
	  ...
	  // now, get association key database
	  auto & ecDB = Common::EmployeeCompanyKeyDB::getInstance();
